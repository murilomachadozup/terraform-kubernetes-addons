locals {
  ...
  scaleway_webhook_dns = merge(
    local.helm_defaults,
    {
      name                        = "scaleway-webhook-dns"
      chart                       = "/home/rguichard/particule/scaleway/cert-manager-webhook-scaleway/deploy/scaleway-webhook"
#        repository                  = "https://k8s.scw.cloud/helm-charts"
      enabled                     = false
      chart_version               = "v0.0.1"
      version                     = "v0.0.1"
    },
    var.scaleway_webhook_dns
  )
...
resource "helm_release" "scaleway-webhook-dns" {
  count                 = local.scaleway_webhook_dns["enabled"] ? 1 : 0
  #repository            = local.scaleway_webhook_dns["repository"]
  name                  = local.scaleway_webhook_dns["name"]
  chart                 = local.scaleway_webhook_dns["chart"]
  version               = local.scaleway_webhook_dns["chart_version"]
  timeout               = local.scaleway_webhook_dns["timeout"]
  force_update          = local.scaleway_webhook_dns["force_update"]
  recreate_pods         = local.scaleway_webhook_dns["recreate_pods"]
  wait                  = local.scaleway_webhook_dns["wait"]
  atomic                = local.scaleway_webhook_dns["atomic"]
  cleanup_on_fail       = local.scaleway_webhook_dns["cleanup_on_fail"]
  dependency_update     = local.scaleway_webhook_dns["dependency_update"]
  disable_crd_hooks     = local.scaleway_webhook_dns["disable_crd_hooks"]
  disable_webhooks      = local.scaleway_webhook_dns["disable_webhooks"]
  render_subchart_notes = local.scaleway_webhook_dns["render_subchart_notes"]
  replace               = local.scaleway_webhook_dns["replace"]
  reset_values          = local.scaleway_webhook_dns["reset_values"]
  reuse_values          = local.scaleway_webhook_dns["reuse_values"]
  skip_crds             = local.scaleway_webhook_dns["skip_crds"]
  verify                = local.scaleway_webhook_dns["verify"]
  values = []
  namespace = kubernetes_namespace.cert-manager.*.metadata.0.name[count.index]
...
data "kubectl_path_documents" "cert-manager_cluster_issuers" {
  ...
  vars = {
    ...
    ingress_class = local.cert-manager["ingress_class"]
  }
}
...
resource "kubectl_manifest" "cert-manager_cluster_issuers" {
  count     = local.scaleway_webhook_dns["enabled"] && local.cert-manager["enable_default_cluster_issuers"] ? length(data.kubectl_path_documents.cert-manager_cluster_issuers.documents) : 0
